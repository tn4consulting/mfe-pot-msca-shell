...
helm --kube-context "kind-$CLUSTER_NAME" upgrade --install msca-shell charts/msca-shell \
...
kubectl --context "kind-$CLUSTER_NAME" rollout restart deployment/msca-shell
kubectl --context "kind-$CLUSTER_NAME" rollout status deployment/msca-shell --timeout=60s
